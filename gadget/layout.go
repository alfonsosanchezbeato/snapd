...
func layoutVolumeStructures(volume *Volume) (structures []LaidOutStructure, byName map[string]*LaidOutStructure, err error) {
	...
	// state of the installation disk and we do not now at image
	// preparation/snap pack time, we need some value for StartOffset so we
	// can perform some validations.  We will overwrite the final offsets
	// later.
