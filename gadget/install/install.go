...
func createPartitions(model gadget.Model, gadgetRoot, kernelRoot, bootDevice string, options Options,
	...
	gadgetToDiskStruct, err := gadget.EnsureVolumeCompatibility(bootVol, diskVolume, nil)
	if err != nil {
	...
	timings.Run(perfTimings, "create-partitions", "Create partitions", func(timings.Measurer) {
		...
		created, err = createMissingPartitions(diskVolume, laidOutBootVol, gadgetToDiskStruct, opts)
	})
	if err != nil {
