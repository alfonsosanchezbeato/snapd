...
`
	...
	_, err = gadget.DiskTraitsFromDeviceAndValidate(lvol, "/dev/foo", nil)
	c.Assert(err.Error(), Equals, `volume foo is not compatible with disk /dev/foo: cannot find disk partition /dev/foo2 (starting at 1052672) in gadget: on disk size 4096 (4 KiB) is not in the valid interval defined by the gadget: min: 1572864000 (1.46 GiB) max: unbounded`)
