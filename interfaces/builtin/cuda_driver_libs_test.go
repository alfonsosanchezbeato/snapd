...
const cudaDriverLibsConsumerYaml = `name: cuda-consumer
version: 0
plugs:
  cuda:
...
func (s *CudaDriverLibsInterfaceSuite) TestLdconfigSpec(c *C) {
	...
	c.Check(spec.LibDirs(), DeepEquals, map[string]bool{
		"/snap/cuda-provider/5/lib1": true, "/snap/cuda-provider/5/lib2": true})
}
...
func (s *CudaDriverLibsInterfaceSuite) TestStaticInfo(c *C) {
	...
	c.Assert(si.Summary, Equals, `allows exposing CUDA driver libraries to the system or snaps`)
