...
// if matching the passed ID, for checking conflicts purposes.
...
// CheckSetKernelCmdlineConflict checks that no active change other
// than ignoreChangeID has a task that touches the kernel command
// line.
func CheckSetKernelCmdlineConflict(st *state.State, ignoreChangeID string) error {
	// check whether there are other changes that need to run exclusively
	if err := checkChangeConflictExclusiveKinds(st, "", ignoreChangeID); err != nil {
		return err
	}
