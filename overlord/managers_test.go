...
func (s *mgrsSuite) TestHappyRemoteInstallAndUpdateManyWithEpochBump(c *C) {
	...
	affected, tasksets, err := snapstate.InstallMany(st, snapNames, 0, &snapstate.ManyFlags{})
	c.Assert(err, IsNil)
	...
	affected, tasksets, err = snapstate.UpdateMany(context.TODO(), st, nil, 0, &snapstate.Flags{}, &snapstate.ManyFlags{})
	c.Assert(err, IsNil)
	sort.Strings(affected)
	c.Check(affected, DeepEquals, snapNames)
...
`
	...
	updated, tss, err := snapstate.UpdateMany(context.TODO(), st, []string{"foo"}, 0, nil, &snapstate.ManyFlags{})
...
`
	...
	// refresh all
	updated, tss, err := snapstate.UpdateMany(context.TODO(), st, nil, 0, nil, &snapstate.ManyFlags{})
	c.Assert(err, IsNil)
	c.Assert(updated, DeepEquals, []string{"foo"})
	c.Assert(tss, HasLen, 2)
	verifyLastTasksetIsRerefresh(c, tss)
...
`
	...
	affected, tss, err := snapstate.UpdateMany(context.TODO(), st, nil, 0, nil, &snapstate.ManyFlags{})
	c.Assert(err, IsNil)
...
version: @VERSION@`
	...
	updates, tts, err := snapstate.UpdateMany(context.TODO(), st, []string{"core", "some-snap", "other-snap"}, 0, nil, &snapstate.ManyFlags{})
	c.Assert(err, IsNil)
...
version: 1`
	...
	updates, tts, err := snapstate.UpdateMany(context.TODO(), st, []string{"some-snap"}, 0, nil, &snapstate.ManyFlags{})
	c.Assert(err, IsNil)
...
func (s *mgrsSuite) testUpdateKernelBaseSingleRebootSetup(c *C) (*boottest.RunBootenv20, *state.Change) {
	...
	affected, tss, err := snapstate.UpdateMany(context.Background(), st, []string{"pc-kernel", "core20"}, 0, nil, &snapstate.ManyFlags{})
	c.Assert(err, IsNil)
...
volumes:
	...
	affected, tasksets, err := snapstate.UpdateMany(context.TODO(), st, []string{"pi"}, 0, &snapstate.Flags{}, &snapstate.ManyFlags{})
	c.Assert(err, IsNil)
	sort.Strings(affected)
	...
	affected, tasksets, err = snapstate.UpdateMany(context.TODO(), st, []string{"pi-kernel"}, 0, &snapstate.Flags{}, &snapstate.ManyFlags{})
	c.Assert(err, IsNil)
	sort.Strings(affected)
