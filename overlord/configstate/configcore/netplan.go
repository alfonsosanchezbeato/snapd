...
func handleNetplanConfiguration(tr RunTransaction, opts *fsOnlyContext) (err error) {
	...
	for _, hint := range []string{"0-snapd-defaults", "90-snapd-config"} {
		var wasSet bool
		if err := netplanCfgSnapshot.Call("io.netplan.Netplan.Config.Set", 0, "network=null", hint).Store(&wasSet); err != nil {
			return fmt.Errorf("cannot clean netplan config %s: %v", hint, err)
		}
		if !wasSet {
			return fmt.Errorf("cannot clean netplan config %s: no specific reason returned from netplan", hint)
		}
	}
