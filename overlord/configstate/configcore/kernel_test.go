	...
	s.overlord.TaskRunner().AddHandler("update-gadget-cmdline",
		func(task *state.Task, tomb *tomb.Tomb) error {
			...
			for _, optName := range []string{
				configcore.OptionKernelCmdlineAppend,
				configcore.OptionKernelDangerousCmdlineAppend} {
				...
				if shouldHaveBeenSet {
					if err := task.Get(optName, &taskCmdline); err != nil {
						return fmt.Errorf("error retrieving option in test handler: %v", err)
					}
					if taskCmdline != cmdline {
						return fmt.Errorf("task cmdline is %q, expected %q",
							taskCmdline, cmdline)
					}
				} else {
					if err := task.Get(optName, &taskCmdline); err == nil {
						return fmt.Errorf("%q should not be set", optName)
					}
				}
			}
...
func (s *kernelSuite) TestConfigureKernelCmdlineDangGrade(c *C) {
	s.testConfigureKernelCmdlineHappy(c,
		[]cmdlineOption{{
			name:    configcore.OptionKernelCmdlineAppend,
...
func (s *kernelSuite) TestConfigureKernelCmdlineDangGradeDangCmdline(c *C) {
	s.testConfigureKernelCmdlineHappy(c,
		[]cmdlineOption{{
			name:    configcore.OptionKernelDangerousCmdlineAppend,
			cmdline: "par=val param"}},
		"dangerous")
}
